...
inline FLOAT4 EncodeBGRM8(FLOAT4 rgba)
{
    const FLOAT1 maxVal = max(rgba.r, max(rgba.g, rgba.b));
    const FLOAT1 MaxRange = 20.0f;
    const FLOAT1 MaxValue = 255.0f * MaxRange;
    const FLOAT1 M = max(maxVal * 255.0f, 1.0f);
    const FLOAT1 scale = 1.0f / log2(MaxValue);
    const FLOAT1 m = 255.0f / M;
    FLOAT4 rgbm;
    rgbm.rgb = rgba.rgb * m;
    rgbm.a = log2(M) * scale;
    return rgbm;
...
inline FLOAT4 DecodeBGRM8(in FLOAT4 rgbm)
{
    FLOAT4 r;
    const FLOAT1 MaxRange = 20.0f;
    const FLOAT1 MaxValue = 255.0f * MaxRange;
...
inline FLOAT4 DecodeBGRE8(in FLOAT4 rgbe)
{
    FLOAT4 r;
    r.a = rgbe.a * 255 - 128;
...
inline FLOAT4 DecodeBGRD8(in FLOAT4 rgbd)
{
    FLOAT4 r;
    const FLOAT1 MaxRange = 20.0f;
    const FLOAT1 MaxValue = 255.0f * MaxRange;
...
inline FLOAT3 ParallaxOffset(HALF1 height, HALF3 viewDir, HALF3 normal, HALF1 scale)
{
...
inline FLOAT3 ParallaxOcclusion(sampler2D heightMap, FLOAT3 viewDirTS, FLOAT3 normalTS, FLOAT2 uv, FLOAT1 scale, FLOAT1 lod)
{
    //Source: http://members.gamedev.net/JasonZ/Parallax/Parallax_Occlusion_Mapping.txt
    const FLOAT1 fHeightMapScale = scale;
    ...
    int nNumSamples = lerp(nMaxSamples, nMinSamples, dotEN);
    ...
    while (nCurrSample < nNumSamples)
    {
        ...
        //fCurrSampledHeight = tex2Dgrad(heightMap, uv + vCurrOffset, dx, dy).r;
        FLOAT4 uvLoc = FLOAT4(uv + vCurrOffset.xy, 0, lod);
        fCurrSampledHeight = tex2Dlod(heightMap, uvLoc).a;
...
// inline FLOAT3 ParallaxOcclusion2(sampler2D tangentMapOS, sampler2D normalMapOS, FLOAT3 viewDirTS, FLOAT3 normalTS, FLOAT2 uv, FLOAT scale, FLOAT lod)
// {
//     //Source: http://members.gamedev.net/JasonZ/Parallax/Parallax_Occlusion_Mapping.txt
...
//     FLOAT3 normalOS = tex2Dlod(normalMapOS, uvLoc);
//     FLOAT3 tangentOS = tex2Dlod(tangentMapOS, uvLoc);
...
//     {
//         FLOAT4 uvLoc = FLOAT4((uv + vCurrOffset.xy), 0, lod);
//         FLOAT4 normalOS = tex2Dlod(normalMapOS, uvLoc);
//         FLOAT4 tangentOS = tex2Dlod(tangentMapOS, uvLoc);

// _Color("Diffuse Color", Color) = (1, 1, 1, 1)
// _MainTex("Base (RGB) Alpha (A)", 2D) = "white" {}
// _BumpMap("Normalmap", 2D) = "bump" {}
// _DiffCubeIBL("Custom Diffuse Cube", Cube) = "black" {}
// [HideInInspector] _AO("Ambient Occlusion Alpha (A)", 2D) = "white" {}

//#pragma surface surf Lambert noambient
//#pragma glsl
//#pragma target 3.0
// we use the default lighting functions

//#pragma multi_compile LUX_LINEAR LUX_GAMMA
//#pragma multi_compile DIFFCUBE_ON DIFFCUBE_OFF
//#pragma multi_compile LUX_AO_OFF LUX_AO_ON

#define LUX_DIFFUSE

#ifdef DIFFCUBE_ON
    samplerCUBE _DiffCubeIBL;
#endif

#ifdef LUX_AO_ON
    sampler2D _AO;
#endif

// Is set by script
float4 ExposureIBL = float4(1,1,1,1);

struct Input {
    float2 uv_MainTex;
    float2 uv_BumpMap;
#ifdef LUX_AO_ON
    float2 uv_AO;
#endif
    float3 viewDir;
    float3 worldNormal;
    //INTERNAL_DATA
};

void surf(Input IN, inout SurfaceOutputLux o) {
    half4 diff_albedo = tex2D(sDiffMap, IN.uv_MainTex);
    // Diffuse Albedo
    o.Albedo = diff_albedo.rgb * cMatDiffColor.rgb;
    o.Alpha = diff_albedo.a * cMatDiffColor.a;
    o.Normal = (tex2D(sNormalMap, IN.uv_BumpMap));

    //#include "Lux/LuxCore/LuxLightingAmbient.hlsl"
}

...
void LuxSetup(RenderPath@ effectRenderPath)
{
    /// \todo This code is to be replaced by upgraded zones.
    log.Info("LuxSetup");
    if (effectRenderPath is null)
        return;

    const float AmbientHDRScale = 10.0f;

    effectRenderPath.shaderParameters["AmbientHDRScale"] = AmbientHDRScale;
    if (scene_.octree is null)
    {
        log.Info("No octree!");
        return;
    }
    Array<Drawable@>@ drawables = scene_.octree.GetDrawables(BoundingBox(-20, 20), DRAWABLE_GEOMETRY);
    ...
    for (uint i = 0; i < drawables.length; i++)
    {
        ...
        if (smodel !is null)
        {
            ...
            while (mat !is null)
            {
                mat.shaderParameters["AmbientHDRScale"] = AmbientHDRScale;
...
void CreateScene()
{
    ...
    SetEffectRenderPath(effectRenderPath);
    viewport.renderPath = effectRenderPath;
}

void SetEffectRenderPath(RenderPath@ effectRenderPath)
{
    effectRenderPath.Append(cache.GetResource("XMLFile", "PostProcess/Tonemap.xml"));
    effectRenderPath.Append(cache.GetResource("XMLFile", "PostProcess/BloomHDR.xml"));
    effectRenderPath.Append(cache.GetResource("XMLFile", "PostProcess/FXAA3.xml"));
    // Make the bloom mixing parameter more pronounced
    effectRenderPath.SetEnabled("BloomHDR", effectsEnabled);
    effectRenderPath.SetEnabled("Tonemap", effectsEnabled);
    effectRenderPath.SetEnabled("FXAA3", effectsEnabled);

    LuxSetup(effectRenderPath);
}

void LoadScene()
{
     File loadFile(fileSystem.programDir + "Data/Scenes/LuxBedroom.xml", FILE_READ);
...
void CreateStandardScene()
{
    scene_.CreateComponent("PhysicsWorld");
    scene_.CreateComponent("Octree");
    Skybox@ sky = scene_.CreateComponent("Skybox");
    sky.model = cache.GetResource("Model", "Models/Sphere2.mdl");
    sky.material = cache.GetResource("Material", "Materials/Skybox.xml");
    
    Material@ mat = cache.GetResource("Material", "Materials/Mr0.xml");
    //for (int y = 0; y < 1; y++)
    const int count = 21;
    const float spacing = 1.5f;
    const float width = count * spacing;
    ...
    int y = 0;
    {
        for (int z = 0; z < count; z++)
        {
            for (int x = 0; x < count; x++)
            {
                ...
                scene_.AddChild(tp);
                tp.position = Vector3(x * spacing - halfWidth, 1, z * spacing - halfWidth);
                ...
                model.material = m;
                float diff = x / float(count - 1);
                float gloss = z / float(count - 1);// 1.0f;
                float spec = z / float(count - 1);// 1.0f;
                ...
                Node@ textNode = Node();
                tp.AddChild(textNode);
                textNode.position = Vector3(0, 1.5f, 0);
                Text3D@ text = textNode.CreateComponent("Text3D");
                
                text.text = "Diff: " + String(diff) + "\nGloss: " + String(gloss) + "\nSpec: " + String(spec);
                ...
                text.faceCameraMode = FC_ROTATE_XYZ;
            }
        }
    }
    ...
    plane.scale = Vector3(width * 2.0f, width * 2.0f, width * 2.0f);
    plane.position = Vector3(0, 0, 0);
    StaticModel@ model = plane.CreateComponent("StaticModel");
    model.model = cache.GetResource("Model", "Models/Plane.mdl");
    model.material = cache.GetResource("Material", "Materials/MG.xml");
    RigidBody@ body = plane.CreateComponent("RigidBody");
    CollisionShape@ shape = plane.CreateComponent("CollisionShape");
    shape.shapeType = SHAPE_STATICPLANE;

    Node@ zone = scene_.CreateChild();
    zone.position = Vector3(0, 0, 0);
    Zone@ z = zone.CreateComponent("Zone");
    z.boundingBox = BoundingBox(-width * 2.0f, width * 2.0f);
    z.zoneTexture = cache.GetResource("TextureCube", "Textures/speccube.xml");
    ScriptInstance@ si = zone.CreateComponent("ScriptInstance");
    si.CreateObject(cache.GetResource("ScriptFile", "Scripts/LuxProbe.as"), "LuxProbe");

//     Node@ light = scene_.CreateChild();
//     light.position = Vector3(0, 5, 0);
//     Light@ l = light.CreateComponent("Light");
//     l.brightness = 300.0f;
//     l.range = width * 2.0f;
//     l.castShadows = true;
}
...
void HandleUpdate(StringHash eventType, VariantMap& eventData)
{
    ...
    if (ui.focusElement is null)
    {
        if (touchEnabled || !useGyroscope)
        {
            ...
            character.controls.Set(CTRL_RUN, input.keyDown[KEY_SHIFT]);
        }
        ...
        {
            RenderPath@ effectRenderPath = renderer.viewports[0].renderPath;
            effectRenderPath.Load(cache.GetResource("XMLFile", "RenderPaths/Prepass.xml"));
            SetEffectRenderPath(effectRenderPath);
        }
